...
**And here is what enforcement costs when it is wrong.** The coherence-divergence monitor's **measured false-positive rate is 10.42%** — 25 of 240 benign trajectories, Wilson 95% CI [7.16%, 14.92%] ([corpus + method](docs/benchmarks/enforce-vs-observe.md#false-positive-posture), [report](evals/reports/coherence_fp-20260902.md), `make eval-coherence-fp`) — so roughly **one correct run in ten** would be parked at an approval gate. The [per-provenance breakdown](docs/reports/coherence-fp-2026-09.md) is the part worth reading, because 10.42% is a weighted average over three unlike classes: **`real: n=0`** — *no trajectory in this corpus was captured from a real agent run* — against `synthetic_grounded: 6.77%` (13/192, every string drawn from this repository's own `git log` subjects and tool allowlist) and `synthetic_authored: 25.00%` (12/48, hand-written to exercise shapes no harvested text produced). Four of eight shapes never fire at all. **So the headline is a property of a corpus mix this repository chose, not an estimate of what a deployed agent would meet.** That number is on this screen rather than in a footnote because it is the honest cost of being in the path instead of beside it, and because `FERRUMDECK_COHERENCE_MODE=enforce` now **refuses to activate** unless a measurement like it exists and is under 14 days old. It does **not** refuse on the *value* of the rate — through 0.8.17 it did, against a threshold that was the Wilson upper bound of the measurement it gated, so it could never fail; the rate and its cost in parked runs are now reported to whoever flips the switch instead. Enforcing on an unmeasured matcher is an availability risk of unknown size.
...
**Honest scope.** R3 enforcement of a coherence divergence is **opt-in** (`FERRUMDECK_COHERENCE_MODE=enforce`) and **shadow-by-default**. The heuristic is a lexical matcher, and its **measured false-positive rate is 10.42%** (25 of 240 benign trajectories, Wilson 95% CI [7.16%, 14.92%]; `make eval-coherence-fp`, [report](evals/reports/coherence_fp-20260902.md), [per-provenance breakdown](docs/reports/coherence-fp-2026-09.md)) — so roughly **one correct run in ten would be parked at an approval gate**, and gating is a deliberate choice for high-consequence agents (the roadmap's own point: R1 async review is insufficient for critical-infrastructure agents where a harmful action can't be undone after the fact). **`min_confidence` ships at `0.0` — no suppression — and that is a stated position, not a default nobody set.** Through 0.8.17 it shipped at `0.5` against a score that could never fall below `0.6375`, so the threshold was disconnected and no operator could tell. The scale is now a true `[0, 1]` and the knob works (0.5 would cut the false-positive rate to 4.17%), but the default is left wide open because suppressing 60% of detections trades false positives for false negatives and this all-benign corpus cannot price that trade. The mapping above is **machine-checked**: `examples/demo/coherence-drift.py` prints and asserts the `R3 → "DeepMind R3: synchronous block-before-execute"` label and that the `enforce` path gates it, exiting non-zero if that ever stops holding. Several advertised layers remain in-progress — see **[Project Status & Limitations](#project-status--limitations)** before relying on any tier.
