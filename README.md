...
**And here is what enforcement costs when it is wrong.** The coherence-divergence monitor's **measured false-positive rate is 10.20%** — 25 of 245 benign trajectories, Wilson 95% CI [7.01%, 14.63%] ([corpus + method](docs/benchmarks/enforce-vs-observe.md#false-positive-posture), [report](evals/reports/coherence_fp-20260906.md), `make eval-coherence-fp`) — so roughly **one correct run in ten** would be parked at an approval gate. The [per-provenance breakdown](docs/reports/coherence-fp-2026-09.md) is the part worth reading, because 10.20% is a weighted average over three unlike classes: **`real: n=5`** — *five trajectories captured verbatim from real agent runs, and* ***none of them contains a single tool action***, *so they cannot fire the detector at all* — against `synthetic_grounded: 6.77%` (13/192, every string drawn from this repository's own `git log` subjects and tool allowlist) and `synthetic_authored: 25.00%` (12/48, hand-written to exercise shapes no harvested text produced). Five of nine shapes never fire at all. **So the headline is a property of a corpus mix this repository chose, not an estimate of what a deployed agent would meet.** That number is on this screen rather than in a footnote because it is the honest cost of being in the path instead of beside it, and because `FERRUMDECK_COHERENCE_MODE=enforce` now **refuses to activate** unless a measurement like it exists and is under 14 days old. It does **not** refuse on the *value* of the rate — through 0.8.17 it did, against a threshold that was the Wilson upper bound of the measurement it gated, so it could never fail; the rate and its cost in parked runs are now reported to whoever flips the switch instead. Enforcing on an unmeasured matcher is an availability risk of unknown size.
...
- **Indirect prompt injection** ([AgentDojo-style, arXiv:2406.13352](https://arxiv.org/abs/2406.13352), 17 attack / 8 benign): **attack success rate 100% → 0%** (17/17 blocked; Wilson CI [81.6%, 100%]) with **100% benign-task utility retained** (8/8) — every attack stopped, zero false-positives on this corpus.
...
**Honest scope.** R3 enforcement of a coherence divergence is **opt-in** (`FERRUMDECK_COHERENCE_MODE=enforce`) and **shadow-by-default**. The heuristic is a lexical matcher, and its **measured false-positive rate is 10.20%** (25 of 245 benign trajectories, Wilson 95% CI [7.01%, 14.63%]; `make eval-coherence-fp`, [report](evals/reports/coherence_fp-20260906.md), [per-provenance breakdown](docs/reports/coherence-fp-2026-09.md)) — so roughly **one correct run in ten would be parked at an approval gate**, and gating is a deliberate choice for high-consequence agents (the roadmap's own point: R1 async review is insufficient for critical-infrastructure agents where a harmful action can't be undone after the fact). **`min_confidence` ships at `0.0` — no suppression — and that is a stated position, not a default nobody set.** Through 0.8.17 it shipped at `0.5` against a score that could never fall below `0.6375`, so the threshold was disconnected and no operator could tell. The scale is now a true `[0, 1]` and the knob works (0.5 would cut the false-positive rate to 4.08%), but the default is left wide open because suppressing 60% of detections trades false positives for false negatives and this all-benign corpus cannot price that trade. The mapping above is **machine-checked**: `examples/demo/coherence-drift.py` prints and asserts the `R3 → "DeepMind R3: synchronous block-before-execute"` label and that the `enforce` path gates it, exiting non-zero if that ever stops holding. Several advertised layers remain in-progress — see **[Project Status & Limitations](#project-status--limitations)** before relying on any tier.
...
- **Trace→signal loop (HarnessX).** The harness-suggestion governance
  ...
  **The store is a database now** (0.8.14,
  [#46](https://github.com/sattyamjjain/ferrumdeck/issues/46)): dispatched runs
  and committed reports live in one `eval_runs` table, with
  `evals/reports/*.json` as its **import source** rather than a second query
  surface. A suite can be dispatched from the dashboard and the run persists.
  **But no eval executor consumes the queue**, so a dispatched run stays
  `pending` with `queued_at` set and `started_at` null. That is reported rather
  than disguised — the gateway answers `202 Accepted`, never `201`, the body says
  no executor has claimed the run, and the list carries `unclaimed: true`. An
  empty store still returns 501 rather than an empty 200: `eval_ingests` records
  that an ingest happened, which is what keeps "we looked and found none"
  distinct from "we never looked" now that the store is a database the gateway
  cannot start without. (The eval *numbers* themselves are
...
- **Realtime run stream (SSE) — all five events push; replay is per-process.**
  The gateway serves `GET /v1/events/{channel}` and pushes every run-channel
  governance event: `policy.response.recorded`, `policy.decision.explained`,
  `routing.decision.recorded`, `coherence.divergence.detected` and
  `run.forecast.updated`. Each is published only **after** the record it
  describes has committed — from inside the audit write for the four backed by
  `audit_events`, and on the `Ok` arm of an awaited `update_forecast` for the
  forecast, whose record is the run row and which therefore carries `forecast_at`
  rather than a `record_id`. A failed write publishes nothing, because silence is
  the correct signal when there is no record to read back.
  Two limits, both **deliberately accepted** with the reasoning recorded on
  [#47](https://github.com/sattyamjjain/ferrumdeck/issues/47). The replay buffer
  is **per process**, so a reconnect that lands on a different replica of a
  multi-replica gateway cannot be served completely — it emits a `stream.gap`
  naming the range rather than a stream that merely looks quiet. And every SSE
  connection counts against `RATE_LIMIT_PER_MINUTE`: one token per connect, not
  per event, with a correct `Retry-After` on the 429, so a reconnect storm is
  throttled rather than exempted.
  Two of the five have **not been watched arriving on a real socket**:
  `coherence.divergence.detected` needs a multi-step trajectory and
  `routing.decision.recorded` needs a workflow run the dev seed cannot satisfy.
  Both have database-backed ordering tests.
  The BFF *can* emit synthetic events for wire-shape development behind
  `FERRUMDECK_SSE_MOCK_EVENTS`, but that flag is **OFF by default in every
  environment**, so a fabricated enforcement verdict can never reach an
  operator's console.
...
**Automated test coverage.** The CI-gating unit/lint suites total **2,113**
tests, re-derivable with `make claims-recount`: Rust **829**
(`cargo test --workspace -- --list`), Python unit **553** (`pytest` over the four
...
Those counts are *collected*, which is not the same as run. Until 0.8.9 the three
...
- **AP2 Signed-Mandate Spend Gate (autonomous payments, 2nd rail)**: The same pre-call spend gate, extended to [Google AP2](https://github.com/google-agentic-commerce/AP2) payments authorized by a signed Mandate chain. `evaluate_ap2_payment` **verifies the Ed25519 signature chain** — a user-signed `Ap2IntentMandate` (scope: merchants/categories + max amount) and an `Ap2CartMandate` cryptographically bound to it by `intent_id` — then checks the cart against the **intent's authorized scope** and the **same per-task `Budget::has_cost_headroom` ceiling the x402 gate enforces**, all **before** the payment is authorized. **Deny-by-default** on every failure mode (`Ap2DenyKind`): missing/invalid signature, unknown signing key, cart-not-bound-to-intent, non-USD/unpriceable total, merchant/category/amount outside scope, or cart total over the budget ceiling — real signature verification, not a trusted flag. The authorized payment normalizes to an `Ap2CostEvent` that folds into the **same `cost_cents` ledger** as x402 + tokens, and emits the same evidence (W3C-trace-context decision span + audit record; `ferrumdeck.cost.ap2_cents`). Never moves money — verify + gate + record only. Pinned to the real Ed25519 engine by `cargo test -p fd-policy --test ap2_gate`; surfaced as a governed-vs-ungoverned row in [`make bench-governed`](docs/BENCHMARK.md). (`fd_policy::ap2`)
...
- **Realtime run stream (SSE)**: the dashboard subscribes to per-run / per-workspace channels. **⚠ [all five run-channel events push; the replay buffer is per-process](#project-status--limitations)** — the gateway serves `GET /v1/events/{channel}` and pushes all five governance events for real: `policy.response.recorded`, `policy.decision.explained`, `routing.decision.recorded`, `coherence.divergence.detected` and `run.forecast.updated`. Each is published **only after the record it describes has committed**, so an id on the wire is an id that resolves — an event that outran its record could not be told apart from one describing a write that never happened. A synthetic generator can emit them for wire-shape development behind `FERRUMDECK_SSE_MOCK_EVENTS` — **OFF by default in every environment**, so no fabricated enforcement verdict can ever reach an operator's console. ([#47](https://github.com/sattyamjjain/ferrumdeck/issues/47))
...
### Quality
- **Evaluation Framework**: Deterministic offline test suites for agent workflows (the `fd-evals` framework runs the reproducible benchmarks above). **⚠ [read + dispatch live; no eval executor consumes the queue](#project-status--limitations)** — `GET /api/v1/evals/{suites,suites/{id},runs,regression-report}` serve a durable `eval_runs` store, with `evals/reports/*.json` as its import source rather than a second query surface. Every figure carries `measured_at` with its precision and source. A suite can be dispatched from the dashboard and the run persists — but **no executor consumes the queue**, so it answers `202 Accepted` and the run stays `pending`, reported as `unclaimed` rather than dressed up as running ([#46](https://github.com/sattyamjjain/ferrumdeck/issues/46)).
...
- **Baseline Comparisons**: Track performance across versions. **⚠ [read + dispatch live; no eval executor consumes the queue](#project-status--limitations)** — the baseline view reads the same `/api/v1/evals/*` data, which is real and now verified end to end; each regression names the measurement time of both endpoints it compares ([#7](https://github.com/sattyamjjain/ferrumdeck/issues/7)).
