            ...
            <div className="flex-shrink-0 px-6 pt-6 pb-4 border-b border-border/50">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-3 mt-1 text-sm text-muted-foreground">
                                <span className="flex items-center gap-1">
                                    ...
                                    {/* Without the fallback an unrecorded agent version leaves a
                      lone Tag icon with no adjacent text, which reads as a
                      decorative icon rather than as a missing value. */}
                                    {evalRun.agent_version?.trim() || (
                                        <>
                                            <span aria-hidden="true">—</span>
                                            <span className="sr-only">
                                                Agent version not recorded
                                            </span>
                                        </>
                                    )}
                                </span>
                ...
                {gateConfig && GateIcon && (
                    ...
                    >
                        <div className="flex items-center gap-3">
                            <GateIcon
                                className={cn("h-6 w-6", gateConfig.className)}
                            />
                            <div>
                                <div
                                    className={cn(
                                ...
                                <div className="text-sm text-muted-foreground">
                                    Score: {formatPercentage(evalRun.score)}{" "}
                                    (threshold:{" "}
                    ...
                    <div>
                        ...
                        <span className="font-medium">
                            <span className="text-green-400">
                                {evalRun.passed_tasks}
                            </span>
                            {" / "}
                            <span className="text-red-400">
                                {evalRun.failed_tasks}
                            </span>
                            {" / "}
                ...
                {isRunning ? (
                    <div className="flex flex-col items-center justify-center py-16">
                        ...
                        <p className="text-sm text-muted-foreground text-center max-w-md">
                            The evaluation is currently running. Results will
                            appear here automatically when tasks complete.
                        </p>
                        {evalRun.task_results.length > 0 && (
                            <div className="mt-6 text-sm text-muted-foreground">
                                {evalRun.task_results.length} of{" "}
                                {evalRun.total_tasks} tasks completed
                            </div>
                        )}
                    </div>
                ) : (
