...
export default function EvalsPage() {
    ...
    // Each eval tab decides for ITSELF whether its backend is wired (#7). GET
    // /suites now serves the real on-disk suites, while runs and the regression
    // report still return 501 — so this is per-tab, not one page-level notice,
    // or the working Suites tab would be hidden behind the others' 501 state.
    const isNotImplemented = (e: unknown) =>
        e instanceof APIError && e.isNotImplemented;
    const suitesNotImplemented = isNotImplemented(suitesError);
    const runsNotImplemented = isNotImplemented(runsError);
    const regressionNotImplemented = isNotImplemented(regressionError);
            ...
            <div className="flex-shrink-0 px-6 pt-6 pb-4">
                <div className="relative">
                    ...
                    <div className="flex items-center justify-between pb-2">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground">
                                    Test agent performance with automated
                                    evaluation suites
                                </p>
                            </div>
                        </div>
                        ...
                        {mounted && (
                            <Dialog
                                open={runSuiteDialogOpen}
                                onOpenChange={setRunSuiteDialogOpen}
                            >
                                ...
                                <DialogContent>
                                    <DialogHeader>
                                        ...
                                        <DialogDescription>
                                            Select a suite to run. Results will
                                    ...
                                    <DialogFooter>
                                        ...
                                        <Button
                                            onClick={() =>
                                                handleRunSuite(selectedSuiteId)
                                            }
                                            disabled={
                                                !selectedSuiteId ||
                                                runEvalSuiteMutation.isPending
                                            }
                                        >
                                            {runEvalSuiteMutation.isPending
                                                ? "Starting..."
            ...
            <div className="flex-1 min-h-0 px-6 pb-6">
                <Tabs value={selectedTab} onValueChange={setSelectedTab}>
                    ...
                    <TabsContent value="suites" className="mt-6">
                        {suitesNotImplemented ? (
                            <NotImplementedState
                                announce={false}
                                title="Eval suites not available"
                                description="The suites backend returned 501 — the eval suites directory could not be located on this deployment (set FD_EVALS_DIR). This is 'not wired here', not 'no suites'. Tracked in issue #7."
                    ...
                    <TabsContent value="runs" className="mt-6">
                        {runsNotImplemented ? (
                            <NotImplementedState
                                announce={false}
                                title="Eval runs not implemented"
                                description="Listing and running eval suites needs a gateway eval backend, which isn't wired yet. An empty runs table would read as 'no runs have been executed' — this is 'not built yet'. Tracked in issue #7."
                                issueUrl="https://github.com/sattyamjjain/ferrumdeck/issues/7"
                                issueLabel="Track eval backend support on GitHub"
                            />
                        ) : runsLoading ? (
                    ...
                    <TabsContent value="regression" className="mt-6">
                        {regressionNotImplemented ? (
                            <NotImplementedState
                                announce={false}
                                title="Regression report not implemented"
                                description="The regression report needs a gateway eval backend to compute it. A report of '0 regressions' would be indistinguishable from 'we never looked', so none is shown. Tracked in issue #7."
                                issueUrl="https://github.com/sattyamjjain/ferrumdeck/issues/7"
                                issueLabel="Track eval backend support on GitHub"
                            />
                        ) : regressionLoading ? (
                            ...
                            <div className="space-y-6">
                                        ...
                                        <CardContent className="p-4">
                                            ...
                                            <div
                                                className={cn(
                                                    ...
                                                    regressionData.overall_cost_delta_cents >
                                                        0
                                                        ? "text-red-400"
                                                        : regressionData.overall_cost_delta_cents <
                                                            0
                                ...
                                {regressionData.regressions_by_suite.length >
                                0 ? (
                                    <div className="space-y-4">
                                        {regressionData.regressions_by_suite.map(
                                            (suite) => (
