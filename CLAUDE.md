...
make eval-series              # Append new runs to docs/eval-health-series.jsonl
make eval-series-check        # Fail if a published series row was rewritten
...
│     docs/eval-health-series.jsonl # append-only measurement record (never rewritten)
...
- `docs/eval-health.md` is generated from committed reports and regenerated by the nightly; an eval that has never passed is labelled **NEVER PASSED** in its own row rather than omitted. That page is a snapshot; `docs/eval-health-series.jsonl` is the append-only record behind it — one row per eval run with the measurement date, producing commit, numbers and harness version. Rows are keyed to the report that produced them, so a refresh finding nothing new appends nothing rather than restating an old measurement. Never rewrite it: correct by appending a row with `correction_of` and `reason`
