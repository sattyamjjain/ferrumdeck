...
| `smoke` | 2026-09-14 | pass | 1.00 | 32 | 3/3 tasks passed, avg score 1.00, assertion coverage 100% |

## Recent measurements

The last 20 of 60 rows from [`eval-health-series.jsonl`](eval-health-series.jsonl), newest first. The table above says what is true today; this says what was true before, which is the part a snapshot throws away. **A number that has never moved here has not been re-measured** — check `measured` against `recorded` before reading a steady value as a stable one.
...
_Generated 2026-09-14 07:40 UTC._
