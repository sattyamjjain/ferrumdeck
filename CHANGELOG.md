...
- **A confidence-threshold sweep, which found a dead knob.** The report measures
  the rate at two thresholds either side of the shipped `min_confidence = 0.5`.
  It does not move, and the reason is that `compute_confidence` floors at
  **0.6375** at the shipped lookahead of 8, so every threshold at or below that
  admits identical spans. The shipped default sits inside that dead zone; the
  first effective value is above 0.6375. Tracked as an open defect in #55, and
  `test_the_shipped_min_confidence_is_still_inert` fails if the relationship
  changes, pointing at the paragraph that then has to be rewritten.
...
- Three findings from the false-positive work were recovered from merged PR
  ...
  (#56, open), and the false-positive rate resting on **0** real trajectories because
  nothing persists trajectory text (#57). All three stay open; none changes the
  published rate, which reproduces byte-for-byte at 10.42% (25/240).
