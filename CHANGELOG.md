...
### Added
- **The coherence false-positive rate is published per provenance, not just as a
  headline.** `docs/reports/coherence-fp-2026-09.md` splits the 10.42% by where
  each trajectory's text came from and by what the trajectory does: 6.77% on
  `synthetic_grounded` (n=192, strings drawn from this repo's `git log` and the
  `safe-pr-agent` allowlist) against 25.00% on `synthetic_authored` (n=48), with
  `real` printed at **n=0** rather than omitted. Four of eight shapes never fire;
  the headline is carried by `vocabulary_trap_statement` (50%) and
  `handoff_then_unrelated_closure` (33%). A class prints a rate only when its
  Wilson 95% interval is narrower than 40 points — `abandoned_no_closure` (n=7)
  shows its count and no percentage, because one flag there moves the rate 14
  points. The page also states what the corpus is *not*: not real traffic, not a
  precision, not portable to another agent, not a claim about English beyond the
  matcher's own keyword lists. Generated by `make docs-coherence-fp` from the
  committed corpus and the committed report — never from wall-clock time — and
  `make docs-coherence-fp-check` fails when it drifts.
- **A confidence-threshold sweep, which found a dead knob.** The report measured
  the rate at two thresholds either side of the shipped `min_confidence = 0.5`.
  It did not move, and the reason was that `compute_confidence` floored at
  **0.6375** at the shipped lookahead of 8, so every threshold at or below that
  admitted identical spans, and the shipped default sat inside that dead zone.
  **Fixed later in this same release** — see *Fixed* below
  ([#55](https://github.com/sattyamjjain/ferrumdeck/issues/55)); the sweep now
  runs up from the shipped `0.0` and shows a real curve, and the guard test was
  inverted from asserting the defect to asserting the fix.
- **`docs/python-plane.md`** — what the Python data plane is, why it exists in
  Python at all, why it is not on PyPI, and what to build against instead. The
  README stated the non-installability in one line and explained it nowhere,
  which reads as abandoned work rather than as a decision. Records that all six
  package names plus `ferrumdeck` are *free* on PyPI, so the reason is not a name
  collision: these are runtime halves of a deployed system, their versions move
  in lockstep via `bump_version.py` rather than forming a release line, and
...
- **The enforce gate's false-positive threshold was circular, and is now a
  reporting requirement instead.** `MAX_FP_RATE_FOR_ENFORCE = 0.15` in
  `rust/services/gateway/src/coherence_evidence.rs` was the Wilson 95% upper
  bound of the single measurement it gated (10.42%, CI [7.16%, 14.92%]) rounded
  up. A gate derived from the measurement it gates cannot fail against it: a
  detector regressing from 10.42% to 14.9% would still have passed, and had the
  detector improved the threshold would not have followed.

  **No independent source exists, and inventing one was not an option.** An
  honest limit needs gated-run volume and time-to-clear for a parked run — the
  two inputs named in the original code comment. Both live with the operator,
  not in this repository. Any constant chosen without them is an invented number
  wearing the costume of a derivation, which is the species of claim this gate
  exists to stop.

  So the constant and the `RateTooHigh` refusal are removed. Enforcement still
  requires that a measurement **exists** and is **fresh** — both independent of
  what the measurement says, and absence of evidence is still a refusal. The
  rate is then reported to whoever turns the switch on, in operator terms: the
  activation line now carries the rate, the sample size, the Wilson interval,
  the cost in parked runs ("roughly N in every 100 correct runs"), and states
  plainly that there is no threshold and the operator is accepting the rate.
  Two tests pin this: `a_high_rate_is_reported_not_refused` (a 42% rate
  activates, and the line says so) and
  `no_rate_threshold_is_reintroduced_without_an_availability_budget`, which
  fails if rates an order of magnitude apart ever decide differently again.
  `the_threshold_is_the_published_intervals_upper_bound` — a tautology dressed
  as a check — is replaced by one asserting the operator-facing line is complete
  ([#56](https://github.com/sattyamjjain/ferrumdeck/issues/56)).
...
- Three findings from the false-positive work were recovered from merged PR
  descriptions and filed, since PRs #52 and #53 both merged with no review
  comments and the findings existed only inside them: the inert `min_confidence`
  (#55), the enforce threshold `MAX_FP_RATE_FOR_ENFORCE = 0.15` being the Wilson
  upper bound of the measurement it gates rather than an availability budget
  ([#56](https://github.com/sattyamjjain/ferrumdeck/issues/56)), and the
  false-positive rate resting on **0** real trajectories because nothing
  persists trajectory text
  ([#57](https://github.com/sattyamjjain/ferrumdeck/issues/57)). All three are
  addressed in this release — see Added/Fixed/Changed above. None of them moves
  the published rate, which still reproduces byte-for-byte at 10.42% (25/240).
...
- **The integration test asserting this release's central invariant had never run in CI, and its one entry point could not have passed.** `permission_reconstruction.rs` is `#[ignore]`d because it needs a real Postgres, and nothing in CI ran `--ignored` — `make test-integration` is the only caller and no workflow invokes it. That target was also broken: a bare `cargo test --workspace -- --ignored` force-runs the `ignore`-marked doctest in `fd-audit/src/lib.rs`, illustrative pseudo-code referencing the `ferrumdeck` umbrella that cannot compile there, so the target failed for everyone who tried it. And each test returned early *reporting `ok`* when `DATABASE_URL` was unset, so even a correctly wired job could have gone green having connected to nothing — the same vacuous pass the eval harness was caught doing in 0.8.7. All three are closed: `live-stack-tests` runs `cargo test --workspace --lib --tests -- --ignored` against the migrated schema and asserts each of the four tests reported by name (an exit code proves nothing when the skip path passes), the tests now default to the dev database and fail loudly when it is absent, following the convention `fd_storage::migrations`'s own ignored tests already used, and the Makefile target is scoped to match. Found while checking the claim that the invariant "is asserted by a test" — which was true, and did not mean what it sounded like.
