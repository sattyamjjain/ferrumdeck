...
### Changed
- **Workspace version bump**: `0.2.0 → 0.3.0 → 0.4.0` over the lifetime of this pre-release (additive backward-compatible — `[Unreleased]`). Canonical declaration in `Cargo.toml::[workspace.package].version`, mirrored on `fd-dag` (override) and `nextjs/package.json`.
